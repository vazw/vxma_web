...
#VXMA
def VXMA(data, ema_ip, subhag, smooth, atr_input, atrM_input, rsi_ip,aol_ip, m):
    ...
    cBull, cBear = andean(data['Close'], data['Open'], aol_ip, m)
    #alPha rsibb >= 50 ? upT < nz(alPha[1]) ? nz(alPha[1]) : upT : downT > nz(alPha[1]) ? nz(alPha[1]) : downT
    try:
        for i in range(2,m):
...
#    try:
    ...
    data['vxma'] = VXMA(data, ema_ip, subhag, smooth, atr_input, atrM_input, rsi_ip,aol_ip, m)
