...
def bot_setting():
    with sqlite3.connect('vxma.db', check_same_thread=False) as con:
        symbolist = pd.read_sql('SELECT * FROM Bot', con=con)
    return symbolist
...
TIMEFRAMES_DICT = {'1m': '1m', '3m': '3m', '5m': '5m', '15m': '15m',
                   '30m': '30m', '1h': '1h', '2h': '2h', '4h': '4h',
                   '6h': '6h', '8h': '8h', '12h': '12h', '1d': '1d',
                   '3d': '3d', '1w': '1w', '1M': '1M'}
BOTCOL = ['id', 'symbol', 'timeframe', 'ATR', 'ATR_m', 'EMA', 'subhag',
          'smooth', 'RSI', 'Andean', 'Uselong', 'Useshort', 'UseTP',
          'UseTP2', 'UseSL', 'Tail_SL', 'leverage', 'Pivot', 'RR1',
          'RR2', 'TP1', 'TP2', 'Risk', 'maxMargin']
...
def candle(df, symbol, tf):
    ...
    color = mplf.make_marketcolors(up='white',
                                   down='black',
                                   wick='black',
                                   edge='black')
    s = mplf.make_mpf_style(rc=rcs,
                            y_on_right=True,
                            marketcolors=color,
                            figcolor='lime',
                            gridaxis='horizontal')
    try:
        vxma = mplf.make_addplot(data.vxma,
                                 secondary_y=False,
                                 color='blue',
                                 linewidths=0.2)
        mplf.plot(data,
...
async def get_symbol():
    ...
    symbols['datetime'] = pd.to_datetime(symbols['timestamp'],
                                         unit='ms',
                                         utc=True).map(lambda x: x.tz_convert('Asia/Bangkok'))
...
async def fetchbars(symbol, timeframe):
    ...
    exchange = await connect()
    try:
        bars = await exchange.fetch_ohlcv(symbol, timeframe=timeframe,
                                          since=None, limit=barsC)
    except Exception as e:
        print(e)
        await disconnect(exchange)
        await asyncio.sleep(10)
        ...
        bars = await exchange.fetch_ohlcv(symbol, timeframe=timeframe,
                                          since = None, limit =barsC)
    await disconnect(exchange)
    df = pd.DataFrame(bars[:-1], columns=['timestamp', 'Open', 'High',
                                          'Low', 'Close', 'Volume'])
...
async def USESLLONG(df,
    ...
    try:
        if currentMODE['dualSidePosition']:
            orderSL = await exchange.create_order(symbol,
                                                  'stop',
                                                  'sell',
                                                  amount,
                                                  float(low),
                                                  params={'stopPrice': float(low),
                                                      'triggerPrice': float(low),
                                                      'positionSide': Lside})
            if Tailing_SL:
                ordertailingSL = await exchange.create_order(symbol,
                                                              'TRAILING_STOP_MARKET',
                                                              'sell',
                                                              amount,
                                                              params ={'activationPrice': float(RR1(df,
                                                                                                   True,
                                                                                                   ask)) ,
                                                                  'callbackRate': float(indi.callbackRate(df)),
                                                                  'positionSide': Lside})
...
async def USETPLONG(symbol,
                    ...
                    USETP2):
    try:
        orderTP = await exchange.create_ordee(symbol,
                                              'TAKE_PROFIT_MARKET',
                                              'sell',
                                              amttp1,
                                              float(RRTP(df,
                                                         ...
                                                         TPRR1,
                                                         TPRR2)),
                                              params={'stopPrice': float(RRTP(df,
                                                                          True,
                                                                          1,
                                                                          ask,
                                                                          TPRR1,
                                                                          TPRR2)),
                                              'triggerPrice': float(RRTP(df,
                                                                         True,
                                                                          1,
                                                                          ask,
                                                                          TPRR1,
        ...
        if USETP2:
            orderTP2 = await exchange.create_order(symbol,
                                                   'TAKE_PROFIT_MARKET',
                                                   'sell',
                                                   amttp2,
                                                   float(RRTP(df,
                                                   ...
                                                   'triggerPrice':float(RRTP(df,
                                                                             ...
                                                                             2,
                                                                             ask,
                                                                             TPRR1,TPRR2)),'positionSide':Lside})
...
async def OpenLong(df,
    ...
    try:
        ...
        print(f'price : {ask}')
        logging.info(f'Entry Long {symbol} Long @{ask} qmt:{amount}')
        leve = await setleverage(symbol, lev, exchange)
        ...
        if free > min_balance:
            try:
                order = await exchange.createMarketOrder(symbol, 'buy', amount,
                ...
                return
            if USETP:
                await USETPLONG(symbol, df, exchange, ask, TPRR1,
                                TPRR2, Lside, amttp1, amttp2, USETP2)
            if USESL:
                await USESLLONG(df, symbol, exchange,  ask, amount,
                                low, Lside, Tailing_SL, currentMODE)
...
async def USETPSHORT(symbol,
                     ...
                     USETP2):
    try:
        orderTP = await exchange.create_order(symbol,
                                              'TAKE_PROFIT_MARKET',
                                              'buy',
                                              amttp1,
                                              float(RRTP(df,
...
async def USESLSHORT(df,
    ...
    try:
        ...
        print(e)
        notify.send(f'เกิดเตุการณืไม่คาดฝัน Order Stop Loss ทำรายการไม่สำเร็จ {e}')
        logging.info(e)
    return


# OpenShort=Sell
async def OpenShort(df,
                    balance,
                    symbol,
                    lev,
                    exchange,
    ...
    try:
        ...
        logging.info(f'Entry Short {symbol} Short @{bid} qmt:{amount}')
        leve = await setleverage(symbol, lev, exchange)
        ...
        if free > min_balance:
            try:
                ...
                return
            if USESL:
                await USESLSHORT(df, symbol, exchange,  bid, amount,
                                 high, Sside, Tailing_SL, currentMODE)
            if USETP:
                await USETPSHORT(symbol, df, exchange, bid, TPRR1, TPRR2,
                                 Sside, amttp1, amttp2,  USETP2)
            time.sleep(1)
...
async def feed(df,
    ...
    status = pd.DataFrame(current_positions,
                          columns=["symbol",
                                   "entryPrice",
                                   "positionSide",
                                   "unrealizedProfit",
    ...
    if df['BUY'][last] == 1:
        ...
        if is_in_Short:
            print('closeshort')
            await CloseShort(df, balance, symbol, amt, 
                             upnl, exchange, Sside, tf)
        elif not is_in_Long and USELONG:
            await exchange.cancel_all_orders(symbol)
            await OpenLong(df, balance, symbol, leverage,
                           exchange, currentMODE, Lside,
                           tf, RISK, Max_Size, TPPer, TPPer2,
    ...
    if df['SELL'][last] == 1:
        ...
        if is_in_Long:
            print('closelong')
            await CloseLong(df, balance, symbol, amt, upnl, exchange, Lside, tf)
        elif not is_in_Short and USESHORT:
            await exchange.cancel_all_orders(symbol)
            await OpenShort(df, balance, symbol, leverage, exchange,
                            currentMODE, Sside, tf, RISK, Max_Size,
                            TPPer, TPPer2, USETP, USETP2,
                            USESL, Tailing_SL, TPRR1, TPRR2)
            is_in_Short = True
        else:
            print("already in position, nothing to do")
    await disconnect(exchange)
...
async def main():
    global botStatus, insession, t1, t2
    symbolist = bot_setting()
    get_config()
    t2 = time.time()
    seconds = time.time()
    ...
    if str(local_time[12:-9]) == '4:4' and not insession['day']:
        insession['day'] = True
        insession['hour'] = True
    ...
    if len(symbolist.index) > 0:
        ...
        for i in range(len(symbolist.index)):
            try:
                ...
                USETP2      = True if symbolist['UseTP2'][i] == 1 else False
            ...
            await asyncio.gather(feed(df,symbol, tf, RISK, Max_Size, TPPer, TPPer2, USETP, USETP2, USESL, Tailing_SL,TPRR1,TPRR2 ,USELONG,USESHORT,leverage))
...
async def async_main():
    while True:
        task = await asyncio.gather(main())
        print(task)
...
Margin_input = dmc.TextInput(label="MaxMargin", style={"width": 75},id='maxmargin-input', value='%1', type='text')
...
edit_table = dcc.ConfirmDialogProvider([dmc.Button("Edit Data", variant="light",id='edit-table', color='red', n_clicks=0, size="md")],message='ชัวร์แล้วนาาา?',submit_n_clicks=0)
...
def update_VXMA_chart(interval, click, symbol, timeframe, zoom, atr_input, atrM_input, ema_ip, subhag, smooth, rsi_ip, aol_ip, pivot):
    ...
    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms', utc=True).map(lambda x: x.tz_convert('Asia/Bangkok'))
    df = df.set_index('timestamp')
    df = indi.sTreamindicator(df, atr_input, atrM_input, ema_ip, subhag, smooth, rsi_ip, aol_ip, pivot)
...
def excuteBot(click, symbol, timeframe, atr_input, atrM_input,
              ema_ip, subhag, smooth, rsi_ip, aol_ip, leverage, Pivot, RR1, RR2, TP1, TP2, Risk,maxMargin, ready, ul, us, tp, tp2, sl, tsl):
    if click is not None:
        data = pd.DataFrame(columns=BOTCOL)
        if ready:
            id = f'{symbol}_{timeframe}'
            compo = [id, symbol, timeframe, atr_input, atrM_input, ema_ip, subhag, smooth, rsi_ip, aol_ip, ul, us, tp ,tp2, sl, tsl, leverage, Pivot, RR1, RR2, TP1, TP2, Risk, maxMargin]
            data.loc[1] = compo
...
print("──────────────▒█▒▒▒▒███████────THAI───█▒█▒█▒█████▒▒▒▒█▒▒▒███████████───────")
print("───────────────────────────SELF.TAUGHT─────────────────────────────────────")
print("────────────────────────>4WONZEST@GMAIL.COM<───────────────────────────────")


if __name__ == "__main__":
    # th = Thread(target=run)
    # th.start()
    runapp = app.run(debug=True)
    # print('Hello')
    # th.join()
